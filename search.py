...
def keyword_to_titles(metadata):
      ...
      for i in metadata:
          article_title = i[0]
          article_keyword = i[4]
          for j in article_keyword:
              if j in keyword_mapping:
                  keyword_mapping[j].append(article_title)
              else:
                  keyword_mapping[j] = [article_title]
...
def title_to_info(metadata):
      title_to_info_dictionary = {}
      for i in metadata:
          article_title = i[0]
          if article_title not in title_to_info_dictionary:
              title_to_info_dictionary[article_title] = {'author': i[1], 'timestamp': i[2], 'length': i[3] }
      return title_to_info_dictionary
...
'''
  Functions 4-` are called after searching for a list of articles containing the user's keyword.
'''
  # 4) 
  #
...
def article_length(max_length, article_titles, title_to_info):
      ...
      for i in article_titles:
          if title_to_info[i]['length'] <= max_length:
              list_of_article_titles.append(i)
...
def key_by_author(article_titles, title_to_info):
      key_by_author_dictionary = {}
      for i in title_to_info:
          author = title_to_info[i]['author']
          if i in article_titles:
              if author in key_by_author_dictionary:
                  key_by_author_dictionary[author].append(i)
              elif author not in key_by_author_dictionary:
                  key_by_author_dictionary[author] = [i]    
      return key_by_author_dictionary
...
def filter_to_author(author, article_titles, title_to_info):
      ...
      for i in title_to_info:
          if i in article_titles:
              if title_to_info[i]['author'] == author:
                  title_list.append(i)
      return title_list            
...
def filter_out(keyword, article_titles, keyword_to_titles):
      if keyword in keyword_to_titles:
          filtered_list = []
          for j in article_titles:
              if j not in keyword_to_titles[keyword]:
...
def articles_from_year(year, article_titles, title_to_info):
      ...
      for i in article_titles:
          if title_to_info[i]['timestamp'] >= start_unix_timestamp and title_to_info[i]['timestamp'] <= end_unix_timestamp:
